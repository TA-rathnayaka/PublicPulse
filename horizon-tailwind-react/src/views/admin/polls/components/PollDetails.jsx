    ...
    const fetchPoll = async () => {
      if (pollId) {
        ...
        try {
          ...
          // Use options from fetchedPoll
          if (fetchedPoll.options && fetchedPoll.options.length > 0) {
            const formattedOptions = fetchedPoll.options.map((option) => ({
              id: option.id,
            ...
            const votesSum = formattedOptions.reduce((sum, opt) => sum + opt.voteCount, 0);
            console.log("Total Votes:", votesSum);
            setTotalVotes(votesSum);
          } else {
            console.warn("No options found for poll:", fetchedPoll);
            setOptions([]);
            setTotalVotes(0);
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (poll && poll.createdAt) {
      try {
        ...
        if (typeof poll.createdAt === "object" && poll.createdAt.toDate) {
          createTime = poll.createdAt.toDate(); // Firestore Timestamp
        } else {
          // Handle string format, e.g., "March 22, 2025 at 1:18:27 AM UTC+5:30"
          const cleanedDateString = poll.createdAt.replace(/\u202F/g, " ");
          createTime = new Date(cleanedDateString);
        }

        console.log("Parsed createTime:", createTime);

        // Validate Date object
        if (isNaN(createTime.getTime())) {
          console.error("Invalid createdAt date:", poll.createdAt);
          setLastUpdated({ days: 0, hours: 0, minutes: 0 });
          return;
        }
        ...
        console.log("Last Updated:", { days, hours, minutes });
      ...
      console.warn("No createdAt field in poll:", poll);
      setLastUpdated({ days: 0, hours: 0, minutes: 0 });
    }
  }, [poll]);
        ...
        <div className="w-full md:w-1/2 p-4">
          <div className="relative h-full w-full rounded-xl overflow-hidden">
            ...
            {poll.imageUrl ? (
              ...
              >
                <h2 className="text-4xl font-bold text-white">{poll.title || "Poll"}</h2>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-between">
          <div>
            ...
            <div className="mb-6">
              ...
              {options.length === 0 ? (
                <p className="text-gray-500 dark:text-gray-400">No options available for this poll.</p>
              ) : totalVotes === 0 ? (
                <p className="text-gray-500 dark:text-gray-400">No votes have been cast yet.</p>
              ) : (
          ...
          <div className="mt-6">
            <div className="p-6 border border-gray-200 dark:border-white/10 rounded-xl mb-6 bg-white dark:bg-navy-700">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-center">
                Poll ID: {poll.id?.substring(0, 8) || pollId.substring(0, 8)}...
              </p>
            </div>
